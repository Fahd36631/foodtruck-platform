...
export const CustomerAccountScreen = () => {
  ...
  const pastOrdersCount = useMemo(
    () => (ordersQuery.data ?? []).filter((order) => !ACTIVE_ORDER_STATUSES.has(order.status)).length,
    [ordersQuery.data]
  );

  const stats = [{ icon: "time-outline" as const, label: "طلبات سابقة", value: pastOrdersCount }];
        ...
        <View style={styles.profileCard}>
          <View style={styles.profileTop}>
            ...
            <View style={styles.avatarWrap}>
              <Ionicons name="person" size={28} color={ACC.orange} />
            </View>
          </View>
